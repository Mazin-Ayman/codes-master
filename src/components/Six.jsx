...
const SixComp = () => {
  ...
  const handleForm = (e) => {
    ...
    if (role === "customers") {
      ...
      if (depVal >= 200000) {
        let res =
          (200000 * period * (depBrRate + 0.02)) / 365 +
          (depVal - 200000) * ((period * depBrRate) / 365);
        setOutput(res.toFixed(2));
        console.log(res);
      } else {
    ...
    <main className="six">
      ...
      {output && (
        <div className="output">
          <p>
            قيمة الفائدة المحتسبة: <span className="value">{output}</span>
          </p>
          <p>
            سعر العائد الحالي-3 للأجل الأقل:{" "}
            <span className="value">{mydata[depType].depBrRate * 100}%</span>
          </p>
          {specialRate && output && (
            <p>
              سعر العائد المميز للموظفين:{" "}
              <span className="value">{specialRate}%</span>
            </p>
          )}
        </div>
      )}
